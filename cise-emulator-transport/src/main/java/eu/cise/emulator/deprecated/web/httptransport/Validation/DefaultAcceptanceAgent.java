...
import eu.cise.emulator.deprecated.web.httptransport.Exception.IllegalMessageException;
import eu.cise.emulator.deprecated.web.httptransport.Exception.InvalidMessageSignatureException;
...
public class DefaultAcceptanceAgent implements TreatIncomingAgent {
    ...
    private Acknowledgement signatureError(InvalidMessageSignatureException e, String payload) {
        return buildAck().ackCode(BAD_REQUEST)
                .ackDetail(format("XML Invalid Signature. WebSocketMessage sender not verified. %s\n\n%s",
